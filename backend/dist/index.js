...
apiRouter.get('/files/download/:cid', async (req, res) => {
    try {
        ...
        const normalizeCid = (c) => c.replace(/^ipfs:\/\//, '').replace(/^\/?ipfs\/?/, '');
...
apiRouter.post('/items/delete', async (req, res) => {
    try {
        ...
        const objectItemIds = itemIds.map((id) => id);
...
apiRouter.put('/contacts/:contactId', async (req, res) => {
    try {
        ...
        const result = await contactsCollection.updateOne({ _id: contactId, owner: owner }, { $set: { name, address } });
        if (result.matchedCount === 0) {
