...
const findOrCreateUser = async (address) => {
    ...
    if (!user) {
        ...
        await db.run('INSERT INTO users (address, storageLimit, storageUsed) VALUES (?, ?, ?)', user.address, user.storageLimit, user.storageUsed);
...
apiRouter.post('/files/metadata', async (req, res) => {
    try {
        ...
        await db.run('INSERT INTO files (_id, filename, cid, size, fileType, owner, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?)', newFile._id, newFile.filename, newFile.cid, newFile.size, newFile.fileType, newFile.owner, newFile.createdAt);
...
apiRouter.get('/files/:ownerAddress', async (req, res) => {
    try {
        ...
        const sharedCids = sharedCidsResult.map((s) => s.cid);
        let sharedFiles = [];
